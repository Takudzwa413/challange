...
weatherWidget.init = function (settings) {
  ...
  Promise.all([this.getWeather(), this.getForecast()]).then((resolve) => {
    ...
    document.getElementsByClassName("ow-temp-current")[0].innerHTML =
      Math.round(weather.main.temp) + "&deg";
    document.getElementsByClassName("ow-pressure")[0].innerHTML =
      weather.main.pressure + " hPa";
    document.getElementsByClassName("ow-humidity")[0].innerHTML =
      weather.main.humidity + "%";
    document.getElementsByClassName("ow-wind")[0].innerHTML =
    ...
    for (let f of forecast) {
      ...
      if (!!fs[date]) {
        fs[date].temp_max =
          f.main.temp_max > fs[date].temp_max
            ? f.main.temp_max
            : fs[date].temp_max;
        fs[date].temp_min =
          f.main.temp_min < fs[date].temp_min
            ? f.main.temp_min
            : fs[date].temp_min;
    ...
    let counter = 0;
    for (let day in fs) {
      let icon = this.settings.icon_mapping[
        this.getIconWithHighestOccurence(fs[day].icons)
      ];
      let fi = forecast_items[counter];
      fi.getElementsByClassName("max")[0].innerHTML =
        Math.round(fs[day].temp_max) + "&deg";
      fi.getElementsByClassName("min")[0].innerHTML =
        Math.round(fs[day].temp_min) + "&deg";
